...
def load(ckpt_dir, net, optim):
    ...
    epoch = int(ckpt_lst[-1].split("epoch_")[-1].split(".pth")[0])
    ...
    with torch.no_grad():
        ...
        for batch, data in enumerate(loader_test, 1):
            ...
            for j in range(label.shape[0]):
                ids = num_batch_test *(batch-1) + j
               
                plt.imsave(os.path.join(result_dir, "png", f"label_{ids:04d}.png"), label[j].squeeze(), cmap="gray")
                plt.imsave(os.path.join(result_dir, "png", f"input_{ids:04d}.png"), inputs[j].squeeze(), cmap="gray")
                plt.imsave(os.path.join(result_dir, "png", f"output_{ids:04d}.png"), output[j].squeeze(), cmap="gray")

                np.save(os.path.join(result_dir, "numpy", f"label_{ids:04d}.np"), label[j].squeeze())
                np.save(os.path.join(result_dir, "numpy", f"input_{ids:04d}.np"), inputs[j].squeeze())
                np.save(os.path.join(result_dir, "numpy", f"output_{ids:04d}.np"), output[j].squeeze())

    print(f"AVERAGE : | BATCH {batch:04d} / {num_batch_test:04d} | LOSS {np.mean(loss_arr):.4f}")
